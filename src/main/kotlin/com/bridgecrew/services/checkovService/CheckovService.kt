...
abstract class CheckovService(val project: Project) {
    ...
    var gitRepo = defaultRepoName //TODO - get from GitUtils
    ...
    fun getExecCommandForSingleFile(apiToken: String, gitRepoName: String, filePath: String): ArrayList<String> {
        ...
        cmds.addAll(getCheckovCliArgsForExecCommand(apiToken, gitRepoName))
    ...
    fun getExecCommandsForRepositoryByFramework(apiToken: String, gitRepoName: String): ArrayList<ArrayList<String>> {
        //docker run --rm --tty --env LOG_LEVEL=DEBUG --volume /Users/mshavit/source/platform:/platform bridgecrew/checkov -f platform/src/microStacks/alertsValidationStack/main.tf -s --bc-api-key '3789f913-f1bb-4da3-9990-70025039932d' -o json
        ...
        baseCmds.addAll(getCheckovCliArgsForExecCommand(apiToken, gitRepoName))

        baseCmds.add("-d")
        baseCmds.add(FilenameUtils.separatorsToSystem(project.basePath!!))
    ...
    protected fun getCheckovCliArgsForExecCommand(): ArrayList<String> {
        ...
        return arrayListOf("-s", "--bc-api-key", apiToken, "--repo-id", defaultRepoName, "-o", "json")
    }
